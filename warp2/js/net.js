// Serverless P2P multiplayer: Trystero (WebRTC, signalling over public Nostr relays).
// The "host" is the peer with the lowest id — deterministic, recomputed on leave.
import { joinRoom, selfId } from 'trystero';

const APP_ID = 'novatix-warp2-v1';
...
export class Net {
  constructor() {
    this.room = null;
    this.peers = new Set();
    this.profiles = new Map(); // peerId -> {name, design, ready}
    // Callbacks wired by the lobby / game.
    this.onPeers = () => {};
    this.onStart = () => {};       // (cfg)
  ...
  join(code) {
    this.leave();
    this.room = joinRoom({ appId: APP_ID }, 'warp-' + code.toUpperCase());
